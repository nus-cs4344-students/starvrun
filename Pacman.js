...
function Pacman(map, isPlayer) {
    ...
    this.checkCollision = function () {
        if ((stuckX === 0) && (stuckY === 0) && dead === false) {
            if (this.inGrid()) {
                ...
                if ((dirX == 1) && (gridAheadX < map.getWidth()))
                    gridAheadX += 1;
                if ((dirY == 1) && (gridAheadY < map.getHeight()))
                    gridAheadY += 1;
                if ((dirX == -1) && (gridAheadX >= 0))
                    gridAheadX -= 1;
                if ((dirY == -1) && (gridAheadY >= 0))
                    gridAheadY -= 1;
                ...
                if ((mapItemAhead === Starvrun.WALL || mapItemAhead === Starvrun.EMPTY)) {
                    ...
                    if ((stuckX == 1) && ((posX % (WIDTH / 2)) !== 0))
                        posX -= speed;
                    if ((stuckY == 1) && ((posY % (WIDTH / 2)) !== 0))
                        posY -= speed;
                }
            }
        }
    ...
    this.runTimers = function () {
        ...
        if ((beastMode_Timer === 0) && (beastMode === true))
            this.disableBeastMode();
        if ((stunned_Timer === 0) && (stunned === true))
            this.disableStunned();
        if ((blinkTimer === 0) && (blinkMode === true))
            this.disableBlinkAnim();
    }
    ...
    this.move = function () {
        ...
        if (!dead) {
            ...
            //boundary checking, ensure Pac can move across other side of map
            if (posX >= map.getWidthPx() - WIDTH / 2)
                posX = speed - WIDTH / 2;
            if (posX <= 0 - WIDTH / 2)
                posX = map.getWidthPx() - speed - WIDTH / 2;
            if (posY >= map.getHeightPx() - WIDTH / 2)
                posY = speed - WIDTH / 2;
            if (posY <= 0 - WIDTH / 2)
                posY = map.getHeightPx() - 4 - WIDTH / 2;
        }
    }
